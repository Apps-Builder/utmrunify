...
class SignupSelokaScreen extends StatefulWidget {
  const SignupSelokaScreen({Key? key}) : super(key: key);

  @override
  _SignupSelokaScreenState createState() => _SignupSelokaScreenState();
}

class _SignupSelokaScreenState extends State<SignupSelokaScreen> {
  bool is5kmSelected = false; // Default not selected
  bool is10kmSelected = false; // Default not selected
  String selectedSize = 'XL - SHORT SLEEVE'; // Default selected size
  double subtotal = 0.0; // Initial subtotal is 0 as no category is selected

  void _updateSubtotal() {
    setState(() {
      subtotal = (is5kmSelected ? 50 : 0) + (is10kmSelected ? 65 : 0);
    });
  }

  String _getEntitlements() {
    if (is5kmSelected) {
      return '1. 5KM Seloka Run finisher jersey\n'
          '2. 5KM finisher medal\n'
          '3. 5KM race bib\n'
          '4. E-certificate\n'
          '5. Refreshments';
    } else if (is10kmSelected) {
      return '1. 10KM Seloka Run finisher jersey\n'
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButton<String>(
              ...
              dropdownColor: Colors.white, // Maroon color for dropdown
              underline: Container(
                height: 2,
                color: Colors.red[900], // Maroon underline
              ),
            ),
            const SizedBox(height: 16),
            Expanded(
